...
## Forbidden Actions
- CANNOT modify clients table (redirect to Strategist)
- CANNOT modify segments table (redirect to Strategist)
- CANNOT modify recipes table (redirect to Strategist)
...
- When pushing to Clay: ALWAYS validate webhook URL and send test lead first
...
- If the operator asks to do something outside their role, say:
  "That's a Strategist task. Please ask Mayank to handle this."
...
- Forgetting to include existing email outputs in the push → ALWAYS include them
